...
def linreg(X, y, reg=0.0):
	...
	matrix[0, 0] = 0
	W_opt = np.linalg.solve(np.transpose(X) @ X + reg * matrix, np.transpose(X) @ y)
...
def find_RMSE(W, X, y):
	...
	differ = y - y_pred
	m = X.shape[0]
	MSE = np.linalg.norm(differ, 2) ** 2 / m
	RMSE = np.sqrt(MSE)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = m) for m in reg_list]
	...
	"*** YOUR CODE HERE ***"
	lambda_index = np.argmin(RMSE_list)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		train_rmse = np.sqrt(np.linalg.norm((X_train @ W).reshape((-1, 1)) + b \
			- y_train) ** 2 / m_train)
		...
		W_grad = ((X_train.T @ X_train + reg * np.eye(n)) @ W + X_train.T @ \
			(b - y_train)) / m_train
...
if __name__ == '__main__':
	...
	X_val = np.column_stack((np.ones_like(y_val), X_val))
